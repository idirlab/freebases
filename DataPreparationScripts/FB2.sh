...
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE freebase_clean_mediator_subject SELECT * FROM freebase_clean f WHERE f.subject IN (SELECT * FROM mediator_entities)" &

#freebase triples with mediator object
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE freebase_clean_mediator_object SELECT * FROM freebase_clean f WHERE f.object IN (SELECT * FROM mediator_entities)" &

wait

#find edges with both mediator-endpoints (these edges will be removed)
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_spo ON freebase_clean_mediator_subject(subject, predicate, object(150))"
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_spo ON freebase_clean_mediator_object(subject, predicate, object(150))"
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE double_mediator_edges SELECT s.subject, s.predicate, s.object FROM freebase_clean_mediator_subject s JOIN freebase_clean_mediator_object o WHERE s.subject=o.subject AND s.object=o.object AND s.predicate=o.predicate"

#removing triples having mediator nodes
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE freebase_clean_no_mediator SELECT * FROM freebase_clean WHERE subject NOT IN (SELECT * FROM mediator_entities) AND object NOT IN (SELECT * FROM mediator_entities)"
...
mysql -u $USERNAME -p$PASSWORD freebase -e "DELETE FROM freebase_clean_mediator_subject  WHERE EXISTS (SELECT * FROM double_mediator_edges WHERE subject=freebase_clean_mediator_subject.subject AND predicate=freebase_clean_mediator_subject.predicate AND object=freebase_clean_mediator_subject.object)"
mysql -u $USERNAME -p$PASSWORD freebase -e "DELETE FROM freebase_clean_mediator_object  WHERE EXISTS (SELECT * FROM double_mediator_edges WHERE subject=freebase_clean_mediator_object.subject AND predicate=freebase_clean_mediator_object.predicate AND object=freebase_clean_mediator_object.object)"

#create index for concatenating edges
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_subject ON freebase_clean_mediator_subject(subject)" &
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_object ON freebase_clean_mediator_object(object(150))" &
wait

#concatenating mediator-subject edges with mediator-subject edges
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE concatenate_edge_pairs_1 SELECT f1.object AS subject, CONCAT(f1.predicate, \"-\", f2.predicate) AS predicate, f2.object AS object  FROM freebase_clean_mediator_subject f1 JOIN freebase_clean_mediator_subject f2 ON f1.subject=f2.subject WHERE f1.predicate > f2.predicate" &

#concatenating mediator-object edges with mediator-object edges
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE concatenate_edge_pairs_2 SELECT f1.subject AS subject, CONCAT(f1.predicate, \"-\", f2.predicate) AS predicate, f2.subject AS object FROM freebase_clean_mediator_object f1 JOIN freebase_clean_mediator_object f2 ON f1.object=f2.object WHERE f1.predicate > f2.predicate" &

#concatenating mediator-subject edges with mediator-object edges
(mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE concatenate_edge_pairs_3_temp SELECT f2.subject AS subject, f2.predicate AS predicate1, f1.predicate AS predicate2, f1.object AS object FROM freebase_clean_mediator_subject f1 JOIN freebase_clean_mediator_object f2 ON f1.subject=f2.object WHERE f1.predicate != f2.predicate" &&
...
#create datagraph
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE freebase_clean_no_mediator_new LIKE freebase_clean_no_mediator"
mysql -u $USERNAME -p$PASSWORD freebase -e "INSERT INTO freebase_clean_no_mediator_new SELECT * FROM freebase_clean_no_mediator"
mysql -u $USERNAME -p$PASSWORD freebase -e "INSERT INTO freebase_clean_no_mediator_new SELECT * FROM concatenate_edge_pairs"
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_predicate ON freebase_clean_no_mediator_new(predicate)"
#################### till here 
...
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE INDEX idx_all ON freebase_clean_no_mediator_new(subject, predicate, object(150))" &
wait
mysql -u $USERNAME -p$PASSWORD freebase -e "CREATE TABLE freebase_datagraph SELECT DISTINCT e1.id AS subject, p.id AS predicate, e2.id AS object FROM freebase_clean_no_mediator_new f JOIN properties_id_label p JOIN entities_id_label e1 JOIN entities_id_label e2 WHERE (f.subject=e1.mid AND f.object=e2.mid AND f.predicate=p.label)"
